...
function themes.fromTextureSet(opts)
  ...
  local decalFrequency = opts.decalFrequency or 0.005
  ...
  if decalFrequency > 0 and ts.wallDecals and #ts.wallDecals > 0 then
    function theme:placeWallDecals(allWallLocations)
      ...
      if decorationCount > 0 then
        ...
        print('\nAll Walls (direction, i, j) are as follows:')
        for i = 1, #allWallLocations do
          print(allWallLocations[i].direction, allWallLocations[i].i, allWallLocations[i].j)
        end
        print('\n')

        print('\nFinally Selected Walls (direction, i, j) for decals:')
        selectedWalls = {24}  -- fill in the wall ids here from all the listed ones on top
        for j = 1, #selectedWalls do
          i = selectedWalls[j]
          print(allWallLocations[i].direction, allWallLocations[i].i, allWallLocations[i].j)
          wallDecals[j] = {
